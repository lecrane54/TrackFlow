 ...
 *
 * @property keyPrefix An optional prefix that is prepended to every property key before
 *   the event is sent to Firebase. For example, a prefix of `"tf_"` would turn a key
 *   `"screen"` into `"tf_screen"`. Pass `null` (the default) to disable prefixing.
 * @property keyMap An optional mapping of original property keys to replacement keys.
 *   Remapping is applied before [keyPrefix]. For example, `mapOf("product_id" to "item_id")`
 *   would rename the `"product_id"` key to `"item_id"` in every event. Pass `null`
 *   (the default) to skip remapping.
 */
class FirebaseProvider(
     ...
     * 4. Applies [keyPrefix] to all property keys.
     */
