 ...
 *   For example, `mapOf("product_id" to "Product ID")` would rename the `"product_id"` key
 *   to `"Product ID"` in every event. Pass `null` (the default) to skip remapping.
 */
class AmplitudeProvider(
     ...
     *
     * The mapper applies [keyMap] remapping to all property keys.
     * The event name is passed through unmodified.
     */
    override val mapper = object : ProviderEventMapper {
        /**
         * Maps an [AnalyticsPayload] to an Amplitude-compatible [ProviderEvent].
         *
         * Property keys are remapped via [keyMap].
