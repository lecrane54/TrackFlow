 ...
 *   Pass `null` (the default) to skip remapping.
 */
class MixpanelProvider(
     ...
     *
     * The mapper applies [keyMap] remapping to all property keys, then appends
     * a `"source" to "trackflow"` entry for attribution purposes.
     */
    override val mapper = object : ProviderEventMapper {
        /**
         * Maps an [AnalyticsPayload] to a Mixpanel-compatible [ProviderEvent].
         *
         * Property keys are remapped via [keyMap] and augmented with an
         * additional `"source": "trackflow"` entry.
         *
